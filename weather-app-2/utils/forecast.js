    ...
    request({url, json: true}, async (err, {body}) => {
        try {
            if (!body) {
                ...
                cb(undefined, `${body.current.weather_descriptions[0]}. In ${body.location.name} current temperature is ${body.current.temperature} and it feeles like ${body.current.feelslike}`)
            }
        } catch (e) {
